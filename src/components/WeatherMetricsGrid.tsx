...
interface WeatherMetricsGridProps {
  ...
  onOpenAQIDetails?: () => void;
  onOpenRainForecast?: () => void;
  language?: Language;
}
...
export const WeatherMetricsGrid: React.FC<WeatherMetricsGridProps> = ({
  ...
  const handleRainClick = () => {
    if (onOpenRainForecast) onOpenRainForecast();
    else if (onOpenDetails) onOpenDetails('rain');
  };

  const handleAqiClick = () => {
    if (onOpenAQIDetails) onOpenAQIDetails();
    else if (onOpenDetails) onOpenDetails('aqi');
  };

  const handleGenericClick = (tab: string) => {
    if (onOpenDetails) onOpenDetails(tab);
    else if (onOpenRainForecast) onOpenRainForecast();
  };
    ...
    <div className="px-5 mt-4">
      <div className="grid grid-cols-4 gap-2.5">
        ...
        <div
          ...
          onClick={() => handleGenericClick('temp')}
        ...
        <div
          ...
          onClick={() => handleGenericClick('humidity')}
