...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({
  ...
  return (
    <nav
      ...
      className="relative shrink-0 w-full z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/80 px-2 py-1.5 flex items-center justify-between shadow-[0_-8px_20px_rgba(15,23,42,0.06)] select-none"
    >
      {/* 1. Home Tab */}
      <button
        id="nav-tab-home"
      ...
      <div className="relative -top-3.5 px-1">
        <button
          ...
          className="w-13 h-13 rounded-full bg-gradient-to-tr from-blue-600 via-sky-500 to-blue-700 text-white flex items-center justify-center shadow-[0_8px_20px_rgba(25,118,210,0.45)] hover:scale-105 active:scale-95 transition cursor-pointer ring-4 ring-white animate-pulse-ring"
      ...
      >
        <div className="relative">
          <AlertTriangle className={`w-5 h-5 ${activeTab === 'news' ? 'text-red-600 stroke-[2.4]' : 'stroke-[2.2]'}`} />
          <span className="absolute -top-1 -right-1.5 flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
          </span>
        </div>
        <span className="text-[10px] mt-0.5 whitespace-nowrap">{t.navNews}</span>
      </button>
    </nav>
  );
};
