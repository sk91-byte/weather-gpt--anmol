...
export const LiveNavigationHUD: React.FC<LiveNavigationHUDProps> = ({
  ...
  // Auto-drive simulation timer with speed multiplier
  useEffect(() => {
    if (!isPlaying) return;
    const driveInterval = setInterval(() => {
      onProgressChange((prev) => {
        const next = prev + 1.2 * speedMultiplier;
        if (next >= 100) {
          setIsPlaying(false);
          if (!completedRef.current) {
            completedRef.current = true;
            if (onJourneyComplete) {
              setTimeout(() => onJourneyComplete(), 600);
            }
          }
          return 100;
        }
  ...
  }, [isPlaying, speedMultiplier, onProgressChange, onJourneyComplete]);
  ...
  const getManeuver = (p: number) => {
    if (p < 25) {
      ...
      return {
        direction: 'left',
        instruction: 'In 400m, turn left onto Elevated Flyover Corridor',
      ...
      return {
        direction: 'right',
        instruction: `In 650m, take exit right towards ${destinationName}`,
        sub: 'Approach corridor • Speed limit: 30 km/h',
        distance: '650 m'
      };
    } else {
      return {
        direction: 'arrive',
        instruction: `Destination on left: ${destinationName}`,
        sub: 'You have arrived safely ahead of the storm!',
        distance: '50 m'
      };
    }
  };
  ...
  return (
    <div className="absolute inset-0 z-30 pointer-events-none flex flex-col justify-between p-3">
      ...
      {simulatedAlertActive && (
        <div className="pointer-events-auto bg-slate-900/98 text-white rounded-3xl p-5 shadow-2xl border-2 border-amber-400 max-w-md w-full mx-auto my-auto animate-in zoom-in-95 duration-200">
          ...
          <h4 className="text-sm font-black text-white mb-1.5">
            "Heavier rainfall is approaching your current route."
          </h4>
          <p className="text-xs text-slate-300 leading-relaxed mb-4">
            Doppler radar indicates storm cloud intensifying over Sector 56 underpass. Checking for safer alternative corridors...
          </p>

          <div className="p-3.5 rounded-2xl bg-emerald-950/80 border border-emerald-500/60 mb-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-black uppercase text-emerald-300 bg-emerald-900/80 px-2 py-0.5 rounded-full">
            ...
            <div className="grid grid-cols-2 gap-2 text-center p-2 rounded-xl bg-black/40 border border-emerald-500/30 mb-2">
              <div>
                <span className="text-[10px] text-slate-400 block font-bold">Current Safety</span>
                <span className="text-sm font-black text-amber-400">65/100</span>
              </div>
              <div className="border-l border-slate-700">
                <span className="text-[10px] text-emerald-400 block font-bold">New Route</span>
                <span className="text-sm font-black text-emerald-300">91/100</span>
              </div>
            </div>

            <p className="text-[11px] text-emerald-100 font-semibold">
              Avoids low-lying underpass; road remains clear and elevated (+3 min).
            </p>
          </div>
      ...
      <div className="pointer-events-auto bg-white/95 backdrop-blur-md rounded-3xl p-4 shadow-2xl border border-slate-200 max-w-lg w-full mx-auto animate-in slide-in-from-bottom duration-200">
        ...
        <div className="flex items-center space-x-2">
          ...
          {/* Demo Speed Multiplier (1x, 2x, 5x) */}
          <div className="flex bg-slate-100 p-0.5 rounded-xl border border-slate-200">
            {[1, 2, 5].map((spd) => (
              <button
                key={spd}
                onClick={() => setSpeedMultiplier(spd)}
                className={`px-1.5 py-1 rounded-lg text-[10px] font-black transition cursor-pointer ${
                  speedMultiplier === spd
